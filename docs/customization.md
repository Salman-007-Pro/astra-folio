...
Edit names and descriptions in `apps/web/src/lib/visual-styles.ts` and treatments in `apps/web/src/styles/visual-styles.css`. The transition picker saves one of five effects: Smooth Fade, Page Fold, Pixel Flip, Curtain, or Circular Reveal. Edit their metadata and controller in `apps/web/src/lib/style-transitions.ts`, and their appearance in `apps/web/src/styles/style-transitions.css`. Pixel Flip paints a decorative tile wave above the settings dialog, with ten square tiles per row and rows calculated from viewport height. The other effects animate browser page snapshots through View Transitions, falling back to a simple content fade when unavailable. Resizing, hiding the tab, or requesting reduced motion cancels an active effect safely; rapid selections keep the latest choice. Reduced motion or the Motion switch disables animated switching and decorative movement. Liquid Glass uses CSS frosted surfaces, optical edge highlights, and moving light rather than physical refraction; Parallax moves decorative backgrounds only. Neither needs an additional graphics library.
