...
Open **Styles** to choose one of fifteen visual treatments or select a palette. **Night Shift** changes mode. Each mode keeps its own palette selection in local storage. The initial mode follows the visitor’s system preference when no choice is saved. `site.defaultPalette` sets the initial palette for both modes. Original is the default visual style. Style selection persists across routes and reloads, independently of the palettes. Choosing a style closes the settings panel to reveal the transformed page. Styles change page composition, navigation, backgrounds, section layouts and materials, including the About, Work, Writing and Contact pages. The saved transition picker offers Pixel Flip (default), Shutter Sweep, Diagonal Slice, Mosaic Bloom, and 3D Blinds. Pixel Flip uses ten square tiles per row and enough rows to cover the viewport. Every effect uses animated overlays and works without the View Transitions API. Motion off and reduced-motion preferences disable decorative animation; Liquid Glass then stays frosted and Parallax stays still.
